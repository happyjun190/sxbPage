...
(function($) {
	...
	function init(param, obj) {
		if (param && param instanceof Object) {
			...
			var defaults = new Object({
				...
				ajax : {
					...
					param : {
						on : true,
						page : 1,
						pageNo : 1 //  这两个参数为了兼容有些接口对于当前页码的参数问题
					},
					ajaxStart : function() {
						return false
					}
			...
			function getDataId() {
				if (options.ajax && options.ajax.dataId) {
					return options.ajax.dataId
				} else {
					return defaults.ajax.dataId
				}
			}
			...
			function createView(page) {
				...
				var varUrl = ajax.url;
				var param = getParam();
				var _dataId = getDataId();
				if(options.ajaxType == "default"){
					sys.ajax({url:varUrl, 
						params :param,
						callback:  function(data) {
							loadPageCount({
								...
								data : eval("data." + _dataId),// TODO
								pageCount : eval("data." + getPageCountId())
							});
							updateView();
						}
					});
				}else if(options.ajaxType == "form"){
					ajaxRequestForm(varUrl, param, function(data) {
							loadPageCount({
							dataType : ajax.dataType,
							callback : ajax.callback,
							data : eval("data." + _dataId),// TODO
			...
			function loadPageCount(options) {
				if (options.dataType) {
					...
					case "json":
						...
						resultPageCount = options.pageCount;
						//resultPageCount = 100;
						//resultPageCount = 100 /// TODO 总的页数，这个现在还没有从后台返回
						break;
					case "xml":
